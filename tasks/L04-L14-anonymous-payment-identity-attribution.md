...
Final reproducible local evidence: `pnpm verify:local` exited 0 on 2026-09-14 after the browser credential correction. It applied 125 migrations, passed 52 isolated PostgreSQL proofs (including cookie identity reuse, expiry replacement, duplicate webhook, authorization→capture, and full-refund attribution), validated 39 fixtures plus 67 OpenAPI paths/74 operations/3 negative operation cases, ran 501 API and 325 web tests, race-enabled Go checks, deterministic overlay/race/load/fault integration checks, and built the three declared images. Synthetic-only fixtures; no raw tokens, provider credentials, customer data, production database, device browser, store, or staging system were used.
