...
**Scope correction 2026-09-14:** YouTube is **not a v1 slice**. `active/launch/00_LAUNCH_SCOPE_AUTHORITY.md` excludes YouTube data/live ingestion, Super Chat, memberships and catch-up summaries from v1, and `FULL-PRODUCT-DEFINITION.md` §1.9 places all YouTube work — including the connect / disconnect / reconnect UI (F06) and revoked-auth surfacing (F07) — in **Phase 4, post-v1**. The wording below calling this a "v1 slice" is superseded; it describes what was built, not what ships at launch. Nothing here may be scheduled, enabled or marketed as a v1 feature.

**Status:** `YouTube slice built and locally test-proven (poller, ingestion, TipIntent short-link flow, ingest-failure admin surface); the poller now issues streamList instead of liveChatMessages.list, itself built and locally test-proven but with wire framing and quota cost both unverified against a real Google project; safe three-failure fallback to list confirmed; bot chat-acknowledgement code exists but ships flag-off and unverified — Google has not approved the chat-write scope; Twitch/Kick remain out of scope per task boundary; nothing here is proven in a deployed environment`
**Level:** L3
