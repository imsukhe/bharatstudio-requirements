...
## QA reachability and payment-intent correction — 2026-09-14

**Authority:** this is a bounded correction to the existing L16 objective, “Turn a tip amount into a chosen interaction.” The user has expressly authorized local implementation; no production deployment or external-provider claim is authorized.

**Finding:** the durable paid-vote tag/tally/overlay path is runtime-wired, but the public tip form sends no interaction fields and no unauthenticated catalogue exposes a creator's active paid-vote options. A hand-crafted tag may also be rejected by the durable tag procedure while the route still creates a generic tip, which violates the viewer-visible meaning of an explicit paid-vote selection.

**Approved local scope:** add one narrowly projected public paid-vote catalogue (definition id, label, and bounded option key/label only); render it as an optional selection on the ordinary public tip form; and make a supplied complete vote selection fail before payment-order creation when the durable tag rejects it. Untagged tips retain their exact existing behavior. No queue binding, moderation rule, visual/config JSON, supporter identity, amounts/tallies, free votes, stickers, arbitrary media, provider call, or second overlay delivery path may enter this contract.

**Data/security/migration impact:** additive `0126` SECURITY DEFINER read function only, with fixed `search_path`, grants limited to `bsa_app`, and a response projection that cannot disclose internal definition configuration. Existing payment-tag validation remains the source of truth and is called before a provider order is created. A stale/closed/cross-channel/unknown option selected by the viewer becomes a redacted `400`; it never turns into an unlabelled paid order. The public endpoint is read-only and rate-limited by the app's existing public route boundary.

**Acceptance/test plan:** disposable SQL proves only active paid definitions/options on the requested channel appear and no other-channel, free-mode, closed, or internal fields leak. API tests prove catalogue shape/unavailability and that an accepted selection tags exactly once, while rejected/throwing tag outcomes return before order creation; untagged order behavior remains covered. Browser tests prove the selection serialization, exact response parsing, malformed-catalogue safe fallback, and no checkout starts for a rejected selected interaction. Full deterministic verifier, build/image checks, diff check, and source-boundary review are required before evidence is recorded.

**Rollback:** disable/remove the optional picker and public read route; the additive function can remain inert. No existing payments, votes, queues, alerts, identities, or overlay history are rewritten. Browser/OBS, staging, provider checkout, migration rollback rehearsal, and independent review remain external gates.
