...
| 6 | **Safe Soundboard Alert** | Approved clips only, with cooldown and queue. *Owner decision 2026-09-17: sound comes from **both** a first-party clip set we author **and** creator uploads, and a creator's own uploaded clip plays **without any review step** — uploading to your own soundboard is publishing your own content to your own stream, exactly as every other creator-authored asset already works, and naming a review queue nobody staffs would be worse than having none. The upload path reuses the creator-pack asset storage built for the sticker catalogue (migration `0110`) under §19.1's media rules. **The module name describes the playback being safe for a broadcast, not the content being vetted**: no copy may claim clips are safe, approved, checked or reviewed, and no takedown flow, reporting surface or automated scanning is authorised. Duration and file-size caps have **no decided value and no honest reuse anchor**, so they ship configured but unset — which for an upload path means the upload control is inert until a cap exists. First-party clips are unaffected **by that gate** — but see the third gap below. **Built in PRF-02 slice 7 (2026-09-17).** Migration `0143`; overlay read `app_private.list_overlay_soundboard_play`; Pro+ gate `app_private.soundboard_module_entitled` reusing §30.3's "Sound Moments (catalogue)" row; upload ladder reusing §30.3's "Creator sound uploads" row (— / 5 / 25 / 100). **No clip can actually play:** §19.1 puts bytes in GCS/CDN and no GCS client exists anywhere in the repository, so `playbackUrl` is null in every environment — see `MED-21`'s correction and `reviews/2026-09-17-med-21-gcs-storage-stale-claim-audit.md`. **Creator upload remains behind `MED-15`/§18.3**; every upload is refused `caps_not_configured` today, which is what keeps the built state compatible with that gate. **Third gap, found by hostile review 2026-09-17 (`reviews/2026-09-17-slice-7-hostile-code-review.md`): the first-party catalogue has no reachable way to be populated.** `app_private.import_soundboard_catalogue_entry` is defined and granted to `bsa_app`, but **no product code calls it** — no route, no seed, no migration-level call — and `0143` describes its caller as *"a trusted internal import path"* that does not exist. It copies `0110`'s sticker importer faithfully, and that one has the same property, so this is consistent existing practice rather than a new defect. The consequence stands: `soundboard_catalogue_entries` is empty everywhere, so **this module renders nothing in any environment today**, for three independent reasons rather than the two previously recorded. Recorded in `reviews/2026-09-17-remaining-eight-modules-and-youtube-v1-amendment.md`.* |
...
| 14 | **Vertical Stream Layout** | Narrow chat, compact goal, QR, reactions for mobile scenes. *Owner decision 2026-09-17: ship **one fixed 9:16 arrangement with no variant selection**, rather than waiting on `CST-08` (the layout-variant system, Phase 2). **Risk accepted and stated:** `CST-08` may later define variants in a shape this fixed layout does not fit, making it a special case to unwind; the owner took that trade to have vertical output in v1. It is a pure renderer on the existing module contract — one transport, one rAF loop, `transform`/`opacity` only — and introduces no new runtime concept. Tier is **Pro** (§30.3 binding). Recorded in `reviews/2026-09-17-remaining-eight-modules-and-youtube-v1-amendment.md`.* **Built in PRF-02 slice 7 (2026-09-17), and NOT as a module.** Migration `0147`. The prior task record predicted no migration; that was wrong and is corrected in `active/tasks/PRF-02-slice-7-vertical-layout.md`. **A layout is not a module:** `vertical_stream_layout` was already a key in `0131`'s catalogue, and reusing it would have cost a §30.3 cap slot — a Pro creator with five slots would spend one on *being vertical*. It registers no module and consumes no slot. The **Pro+ gate is server-side**, reusing `app_private.current_channel_tier` (migration `0086`) and called from inside the overlay read, so a sub-Pro channel receives **horizontal** rather than an error — a gate in the browser would be UI hiding, which `active/launch/00_LAUNCH_SCOPE_AUTHORITY.md` rejects. It arranges exactly three modules — compact goal, QR, reactions — with **no chat and no reserved gap**, since chat was never a canvas module. `0147` also **retires four dead keys** from `0131`'s constraint (`now_playing`, `chat`, `stream_health_widget`, `vertical_stream_layout`): today's decisions made the first three permanently dead, and a creator could otherwise enable one, consume a cap slot and render nothing forever. **That retirement is destructive** — it deletes matching rows before tightening the constraint, reports the count by `raise notice`, and its rollback block states that deleted rows are not recoverable. Zero rows existed locally. |
...
| 20 | **Media / Meme Queue** | Curated, approved assets only. *Owner decision 2026-09-17: **creator-only. Viewers cannot submit**, and `MED-20` is written to record exactly that. Viewer submission would make us a host of viewer-supplied media at broadcast volume — the same rights, storage and takedown posture as the soundboard uploads, but at far higher volume and with no relationship to the person submitting. No submission endpoint, no approval queue, no viewer-facing surface. Adding one is a new decision, not an extension. **Built in PRF-02 slice 7 (2026-09-17).** Migration `0146`. ~~The card lists the queue and cannot show or play the media.~~ **CORRECTED 2026-09-17 by hostile review (`reviews/2026-09-17-slice-7-hostile-code-review.md`): that statement was FALSE of the code that shipped.** It described the discarded duplicate implementation; the one that landed assigned a database-supplied `storageUrl` straight to `<img>.src`/`<video>.src`, and `storage_url` was validated only as an `https://` prefix — **no host allowlist, so the Canvas would fetch and render bytes from any host on the internet**, against §9.1.1 (*"never... an arbitrary third-party browser-source URL... Ever"*) and §19.1 (*"signed URLs... no public bucket path"*). **Fixed in migration `0148`** by mirroring `0143`'s pattern: a scheme-less, host-less object key resolved server-side against the server's own CDN base. The honest consequence is that this module **joins the other three as unable to display media until GCS/CDN exists** — it had only appeared to work by bypassing the storage architecture. The structural no-submission guard was **mutation-tested rather than asserted**: a `viewer_id` token was injected into `enqueue_media_queue_item`, the test observed to fail, and the file restored byte-identical against `sha256 aa41d36d…`. Recorded in `reviews/2026-09-17-remaining-eight-modules-and-youtube-v1-amendment.md`.* |
...
> **Configuration changes require an overlay reload; only module DATA is live.** Recorded
> 2026-09-17 after hostile review (`reviews/2026-09-17-slice-7-hostile-code-review.md`), because
> nothing stated it anywhere. Module **data** re-reads on every (re)connect through
> `connection.subscribe()`. Module **entitlement** and the **canvas layout** are each read once
> on mount and never again, so a creator who enables a module or switches to the vertical layout
> sees no change until the OBS browser source restarts. The two behave identically, so this is
> not a defect introduced by the layout — but a creator will reasonably expect a toggle to take
> effect live, and today it does not. Making either live is a separate decision: it would put a
> configuration read on the reconnect path, which §4.4.6's "no fetch on page load just in case"
> and §12.7's bounded-data rule both bear on.
...
> **STATUS ANNOTATION, 2026-09-17 — this table describes a TARGET, not a present state.**
> Audited at `bharatstudio-alerts` `932a87b`: none of the Migration, Serving or Rollback rows
> above is implemented. There is no GCS client, no bucket, no credential, no signed-URL code
> and no per-channel switch flag; no migration carries a GCS column. Every media byte in the
> product is in Postgres `bytea` today. The **decision stands and is not reopened** — GCS/CDN
> for bytes with Postgres holding metadata is the design — but nothing here may be read as
> evidence of a built capability, and the 2026-09-14 amendment to
> `active/launch/01_MASTER_RELEASE_AUTHORITY.md` records an approved decision, not a shipped
> one. Three PRF-02 slice-7 modules (#6 audio, #11 logo, #20 media) each shipped the half that
> does not depend on storage and named the half that does. A prerequisite task is required and
> may be externally blocked with `ENV-03/04/05/06/08`. Audit: `reviews/2026-09-17-med-21-gcs-storage-stale-claim-audit.md`.
...
| MED-21 | Lottie + custom branding upload, Studio-tier, live gate. ~~**`bytea` storage is legacy** — new writes go to GCS per §19.1; the `bytea` path stays as the rollback route until backfill completes~~ **CORRECTED 2026-09-17 — that sentence was a stale claim.** No GCS client, dependency, bucket, credential or signed-URL code exists anywhere in the repository, and **zero migrations carry a `gcs_object_key`, `gcs_bucket` or `storage_backend` column** — not even the schema half. `bytea` is not the legacy path, it is the ONLY path (live in `0067`, `0077`, `0119`, `0122`). §19.1's own "done when" (zero new writes to `bytea`) is not met by any measure. **State letter corrected `U` → `A`:** the decision exists, the implementation does not, and `U` asserted a working capability. The GCS/CDN design itself is NOT reopened and remains correct. Audit: `reviews/2026-09-17-med-21-gcs-storage-stale-claim-audit.md` | v1 | A | **P0** |
...
| CTL-12 | Marketing sections behind flags. **CORRECTED 2026-09-17: NOT `kind = marketing_section`.** §20.2's `kind` is a taxonomy of *capabilities* (widget, module, feature, hub_lane, lobby_mode, ai_feature); a marketing section is a page region, not something a creator is entitled to. It gets its **own field**, so the registry keeps meaning one thing — the same reason `capacity_class` exists separately (`CTL-14`). Unblocks `CTL-10`/`11`/`12`. Cost: §20.5's "full site behind flags" routes through two places, and `CTL-11`'s "marketing reads the same source" is slightly less literal. `reviews/2026-09-17-three-owner-decisions-goa17-marketing-bootstrap.md` | v1 | A | P1 |
...
| GOA-17 | Sponsor actions: reveal card, ~~log exposure with timestamp for proof-of-delivery~~ **RESOLVED 2026-09-17: the exposure-log half is DROPPED; the reveal-card half stands.** It contradicted the same-day §6 module #11 decision, which migration `0145` enforces by scanning for `count`/`impression`/`exposure`/`shown_at` tokens — an exposure-logging action would fail that guard by design. A sponsor asking "show me it ran" gets nothing from us, which is the cost §6 #11 already accepted. `reviews/2026-09-17-three-owner-decisions-goa17-marketing-bootstrap.md`.  **CONTRADICTION FLAGGED 2026-09-17 — do not build this row as written.** It says *"log exposure with timestamp for proof-of-delivery"*, but the owner decision the same day (§6 module #11, `reviews/2026-09-17-remaining-eight-modules-and-youtube-v1-amendment.md`) **dropped the sponsor exposure log entirely**: the card renders and counts nothing, and migration `0145`'s SQL test scans `information_schema.columns` and every shipped function body for `count`, `impression`, `exposure`, `views`, `shown_at`, `displayed_at` and `duration` so a counter cannot be added without failing. An action that logs exposure would therefore fail that guard by design. The two cannot both stand: either GOA-17's proof-of-delivery log is dropped to match the sponsor decision, or the sponsor decision is revisited **with legal review**, since the reason it was declined was that any number we render becomes a number a sponsor relies on. **Owner decision required; nothing here may be resolved by an implementer.** | v1 | A | P3 |
