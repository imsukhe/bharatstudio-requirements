...
| L14-09 | Confirm a direct anonymous tip or opaque TipIntent in the web UI | The UI mint request carries only the already-known local intent id; a validated opaque receipt link is shown on success, while receipt failure leaves payment-confirmed state intact. |
...
| L14-09 | **Pass, local.** `apps/web/app/tips/receipt-client.test.ts` proves only the known intent UUID crosses the browser receipt boundary, the token is strictly validated, and failures remain non-fatal. `TipForm.tsx` and `t/[token]/TipIntentConfirm.tsx` call the helper only after their durable status parser returns `paid`. Full `pnpm verify:local` exited 0 on 2026-09-14 (125 migrations, 52 SQL proofs, 501 API + 327 web tests, Go race/vet and three image builds). |
